#include <coreguard.h>

int main(void)
{
    const wchar_t *argv[] = {L"cmd.exe", L"/d", L"/c", L"exit 0"};
    cg_run_options options = {0};
    cg_run_result result = {0};
    cg_job_metrics job_metrics = {0};
    int rc;

    options.argv = argv;
    options.argc = 4U;
    options.timeout_ms = 5000U;
    if (cg_run_with_job_metrics(&options, &result, NULL) != -1) {
        return 1;
    }
    rc = cg_run_with_job_metrics(&options, &result, &job_metrics);
    if (rc != 0 || result.status != CG_STATUS_EXITED ||
        !result.has_exit_code || result.exit_code != 0U ||
        !job_metrics.snapshot_available || job_metrics.query_error != 0U ||
        (job_metrics.valid_fields & CG_JOB_METRIC_TOTAL_PROCESSES) == 0U ||
        job_metrics.total_processes < 1U) {
        cg_run_result_free(&result);
        return 1;
    }

    cg_run_result_free(&result);
    return 0;
}
